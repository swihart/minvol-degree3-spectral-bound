...
\hypersetup{
  ...
  citecolor=blue,
  urlcolor=blue,
  pdftitle={A Degree-Three Spectral Refinement of Nishioka's Lower Bound (v0.1.0)},
  pdfauthor={Bruce J. Swihart},
  pdfsubject={Public research draft v0.1.0, released 2026-09-06; three-dimensional Blaschke--Lebesgue problem},
...
\newcommand{\releasedate}{September 6, 2026}
...
independent subject-matter expert has yet reviewed the complete proof. This is
release \releaseversion, dated \releasedate. The repository should therefore
be cited as a draft seeking verification, not as a certified or peer-reviewed
theorem.
\end{minipage}}
\end{center}
...
The proof proceeds as follows. Section~2 separates the degree-three curvature
energy $N_3$ from the higher-mode tail. Section~3 proves the determinant
inequality special to cubic spherical harmonics. Section~4 converts that
estimate into nuclear-norm control and combines it with Nishioka's pointwise
operator-norm constraint to cap $N_3$. Section~5 then combines the cubic cap,
the refined spectral split, and the support-function volume formula.
...
Since
\[
 \frac9{319}=\frac1{22}-\frac1{58},
\]
the same estimate may be written as
\[
 E(h)\le \frac1{22}N-\frac9{319}(N-N_3).
\]
Thus the reduction relative to Nishioka's spectral bound $E(h)\le N/22$ is
exactly the nonnegative correction $9(N-N_3)/319$. It is positive whenever
higher-degree energy is present and vanishes in the pure degree-three case,
which is treated separately below by the improved budget estimate for $N_3$.
\end{proof}

\begin{remark}[Continuing the spectral split]\label{rem:degree-five}
Let
\[
 N_5:=\norm{A_{h_5}}_2^2.
\]
...
\[
 \frac{1}{2(\lambda_\ell-1)}
\]
equals $1/58$ at $\ell=5$ and is at most $1/110$ for every odd
$\ell\ge7$, the splitting can be refined to
\[
 E(h)\le
 \frac1{22}N_3+\frac1{58}N_5
 +\frac1{110}(N-N_3-N_5).
\]
This refinement alone does not improve
Proposition~\ref{prop:spectral-split}. With only
\[
 0\le N_5\le N-N_3
\]
available, the right-hand side is increasing in $N_5$ and is therefore
maximized at $N_5=N-N_3$. Substitution then recovers
\[
 E(h)\le
 \frac1{22}N_3+\frac1{58}(N-N_3).
\]
Thus a useful degree-five refinement requires an additional estimate on
$N_5$, or a coupled constraint involving $N_3$ and $N_5$.
\end{remark}
...
\end{equation}
Integrating \eqref{eq:norm-C-point} over $\Sph$ and substituting
\eqref{eq:quadratic-moments}, we obtain
\begin{align}
 \norm{C}_2^2
 &=36\frac{4\pi}{3}\tau
   -72\frac{8\pi}{15}\tau
   +68\frac{8\pi}{35}\tau \notag\\
 &=\frac{176\pi}{7}\tau.
 \label{eq:C-norm-invariant}
\end{align}
Equivalently, \eqref{eq:nishioka-mode} with $\lambda_3=12$ gives
$\norm{C}_2^2=110\norm{Y}_{L^2}^2$, which yields the same result from the
first identity in \eqref{eq:quadratic-moments}.
...
Table~\ref{tab:moments}. Appendix~\ref{app:pairings} records the contraction
counts and works out representative pure and mixed rows. A complete row-by-row
slot count, including the fixed contractions in all mixed moments, is provided
in Sections~7--8 of the accompanying repository document
\path{rendered/markdown/proof/DEGREE3_TENSOR_IDENTITY.pdf} (editable source:
\path{proof/DEGREE3_TENSOR_IDENTITY.md}); the exact Python and R pairing
enumerators independently reproduce the same table.
...
\section{Discussion: a spectral hierarchy}

The argument above may be viewed as the first level of a hierarchy of
low-degree spectral relaxations. Nishioka's estimate applies the largest mode
coefficient, $1/22$, to the entire curvature budget. The present argument
retains the degree-three quantity
\[
 N_3=\norm{A_{h_3}}_2^2
\]
as an additional variable and obtains a nontrivial bound on it from the special
algebraic structure of $\Harm_3$.

The spectral decomposition itself can be refined indefinitely. For example,
Remark~\ref{rem:degree-five} isolates
\[
 N_5=\norm{A_{h_5}}_2^2
\]
and assigns the smaller coefficient $1/110$ to all remaining odd degrees.
Spectral coefficients alone, however, do not improve the final bound: without
further information, the relaxed optimization places the entire remaining
curvature budget in $\Harm_5$, whose coefficient $1/58$ is the largest among
the remaining modes.

A sharper continuation therefore requires geometric information beyond the
diagonal harmonic decomposition. Two natural possibilities are:
\begin{enumerate}[label=(\roman*),leftmargin=2em]
 \item a degree-five analogue of Proposition~\ref{prop:cubic-budget}, giving a
 nontrivial upper bound on $N_5$; or
 \item a coupled inequality for $(N_3,N_5)$ derived directly from the
 pointwise constraint
 \[
  -\frac d2\Id\preceq A_h\preceq\frac d2\Id.
 \]
\end{enumerate}
The coupled alternative may be stronger, because the degree-three and
degree-five components need not be capable of simultaneously saturating
separate mode-wise estimates. Since $\dim\Harm_5=11$ and the relevant invariant
algebra is more complicated than in degree three, exact symbolic computation
or certified finite-dimensional optimization may be useful. This is consistent
with Nishioka's proposed use of additional variables and certified
computer-assisted estimates \cite{Nishioka2027}.

\section{Reproducibility and disclosure}

\subsection*{Supporting materials and code availability}
The canonical public repository for this work is
\url{https://github.com/swihart/minvol-degree3-spectral-bound}. The versioned
release corresponding to this manuscript is
\url{https://github.com/swihart/minvol-degree3-spectral-bound/releases/tag/v0.1.0}.
Release \releaseversion\ contains the manuscript source and PDF, expanded proof
documentation, exact symbolic Python calculations, independent base-R checks,
build and preflight scripts, and the verification and AI-provenance records
described below.
...
(GPT-5.6 Sol Pro, accessed August--September 2026) extensively to explore proof
...
As a representative mixed row, consider $Y^2\abs v^2$. Label the two copies of
$T$ arising from $Y^2$ by vertices $1,2$ and those arising from $\abs v^2$ by
vertices $3,4$. The scalar contraction in $\abs v^2$ fixes one edge $34$,
leaving ten sphere-coordinate indices. Type $D$ contributes $3!2!=12$ slot
pairings, while type $\mathcal K$ contributes $3!3!2!2!=144$. For type $J$,
two labeled graphs use $34$ as a doubled edge and contribute $72$ each, and
two use it as a single edge and contribute $36$ each, for a total of $216$.
After substituting $\mathcal K=\tau^2/2-J$, the reduced coefficients are
$84$ for $\tau^2$ and $72$ for $J$. Since the spherical moment factor is
$4\pi/11!!$, this gives
\[
 \int_{\Sph}Y^2\abs v^2\dd
 =\frac{4\pi}{11!!}(84\tau^2+72J)
 =\pi\left(\frac{16}{495}\tau^2+\frac{32}{1155}J\right),
\]
which is the second row of Table~\ref{tab:moments}. The remaining mixed rows
follow by the same slot-counting argument with the scalar contractions in
$\abs v^2$ and $\abs M^2$ fixed.
The repository script \path{python/verify_pairing_counts.py} exhaustively
enumerates the pairings and reproduces the table exactly; the base-R script
\path{R/verify_pairing_counts.R} is an independent implementation.
