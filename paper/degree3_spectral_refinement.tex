...
\[
 ...
 \frac{1}{22}-\frac{1}{58},
\]
the estimate may equivalently be written as
\[
 E(h)
 \le
 \frac1{22}N-\frac9{319}(N-N_3).
\]
Thus, at the spectral-splitting stage, the reduction relative to
Nishioka's bound $E(h)\le N/22$ is exactly the nonnegative correction
\[
 \frac9{319}(N-N_3).
\]
This correction is positive whenever higher-degree energy is present
and vanishes when $N=N_3$, equivalently when $h\in\Harm_3$.  The
pure degree-three case is treated separately below by the improved
budget estimate for $N_3$.
