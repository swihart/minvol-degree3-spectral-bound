...
| RC1 | Public v0.1.0 release candidate | Versioned paper, public README wording, release notes, URLs, date, tag-aware workflow, and release-candidate fresh-clone smoke test | Prepared; exact tagged commit requires three green hosted jobs on `main` and on the version tag before publication |
...
release metadata, hosted verification workflow, and fresh-clone smoke testing
are complete. The release-candidate smoke test passed every substantive check
and build; two generated documentation PDFs exhibited trivial binary variation,
which is documented but is not a release blocker. The remaining internal gate
is to obtain three green hosted jobs on the reviewed commit, tag that commit
`v0.1.0`, and require the three-job tag workflow to pass before the GitHub
...
**public research draft seeking independent mathematical verification** after
the reviewed tagged commit has passed the hosted `main` and tag workflows.
